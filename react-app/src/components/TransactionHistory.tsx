...
const TransactionHistory = () => {
  ...
  const [filter, setFilter] = useState<"all" | "donation" | "withdrawal">(
    "all"
  );
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5;
  const { data: allTransactions = [], isLoading } = useRecentTransactions(50);
      ...
      <Card className="transaction-history border-0 p-3 sm:p-4 md:p-6 transition-all">
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between">
            ...
            <ChevronDown
              className={`w-4 h-4 sm:w-5 sm:h-5 text-primary transition-transform ${
                isOpen ? "rotate-180" : ""
              }`}
            />
          </div>
          ...
          {isLoading ? (
            ...
            <>
                        ...
                        <TableCell className="py-2 sm:py-3">
                          <div className="flex items-center gap-1 sm:gap-2">
                            {tx.type === "donation" ? (
                              ...
                              <ArrowUpCircle className="w-4 h-4 sm:w-5 sm:h-5 text-accent" />
                            )}
              ...
              {!isLoading && paginatedTransactions.length > 0 && (
                <div className="mt-3 sm:mt-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3 border-t pt-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    <Button
                      ...
                      className="hover:bg-muted/50 transition-all h-8 w-8 p-0"
                    >
